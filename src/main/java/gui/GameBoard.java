...
public class GameBoard extends JPanel implements Runnable {
    ...
    public void paintComponent(Graphics g) {
        g.clearRect((int) getParent().getBounds().getX(),
                (int)getParent().getBounds().getY(),
                (int)getParent().getBounds().getWidth(),
                (int) getParent().getBounds().getHeight());
        g.setColor(Color.BLACK);
        for (int x = 0; x < this.sizeX; x++) {
            for (int y = 0; y < this.sizeY; y++) {
                if (this.board.getCellAt(x, y)) {
                    g.fillRect(x * SQUARE_SIZE + PADDING, y * SQUARE_SIZE +
                            PADDING, SQUARE_SIZE, SQUARE_SIZE);
                }
            }
        }
        ...
        for (int i = 0; i <= this.sizeX; i++) {
            g.drawLine(i * SQUARE_SIZE + PADDING, PADDING, i * SQUARE_SIZE +
                    PADDING, this.sizeY * SQUARE_SIZE + PADDING);
        }
        for (int j = 0; j <= this.sizeY; j++) {
            g.drawLine(PADDING, j * SQUARE_SIZE + PADDING,
                    this.sizeX * SQUARE_SIZE + PADDING, j *
                            SQUARE_SIZE + PADDING);
        }
    ...
    public class MouseClickListener extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent me) {
            ...
            GameBoard.this.board.changeCellAt(((me.getX() - PADDING)
                    / SQUARE_SIZE),
                    ((me.getY() - PADDING) / SQUARE_SIZE));
